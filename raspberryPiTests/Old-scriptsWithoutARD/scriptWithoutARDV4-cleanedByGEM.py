...
#PROBLEMS:
# - recording saved to dailedNUM + new dailed number Recording VALID. Saving to voicemail-T1-011.wav
# - File not found or skipped: audio/question-1.wav (during VOICEMAIL_RECORDING)
# - Playing audio/ReceiverCall3.wav on T2 (right)... (during INITATION VOICEMAIL)
...
class AudioChannel:
    """Handles audio playback and recording for a specific channel (Left/Right)."""
    def __init__(self, name: str, channel_side: str):
        self.name = name
            ...
            with wave.open(str(filename), 'rb') as wf:
                ...
                while data and not stop_event.is_set():
                    ...
                    if width == 1:
                        ...
                        # 16-bit -> Unpack and interleave with silence
                        count = len(data) // 2
...
class PhoneSystem:
    ...
    def run_case_conversation(self, initiator: Phone):
    ...
    def run_sub_case_conversation_intro(self):
        ...
        if self.receiver.state == PhoneState.OFFHOOK:
            ...
            self.receiver.play_async(["ReceiverOffhookBeforeRing.wav", "ReceiverWaitingTone.wav"])
    ...
    def run_sub_case_conversation_starter(self):
        ...
        dialed_suffix = self.dial_buffer[self.sender.number]
    ...
    def run_sub_case_conversation_ring_timeout(self):
        ...
        self.receiver.play_async(["ReceiverCall3.wav"]) # "Stop ringing"
...
def find_arduinos() -> Dict[str, Any]:
    ...
    if ports:
        ...
        for p in ports:
            ...
            # Heuristic check
            if any(k in p.description for k in ["Arduino", "USB Serial", "usbmodem"]):
                try:
